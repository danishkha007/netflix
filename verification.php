...
if(isset($_SESSION['userLoggedIn'])){
   ...
   $message="{$user['firstName']},Your account has been created. Please visit this link to verify your email <a href='http://localhost/netflix/verification/$link'>Click Here To Verify</a>";
   $subject="[NETFLIX] Please verify your account";
   $check = $loadFromUser->get("verification",["user_id"],["user_id"=>$user_id]);
  if($check==null){
...
if(is_get_request()){
    if(isset($_GET['verify'])){
        ...
        echo $user_id;
        $errors=array();
        $code=FormSanitizer::sanitizeFormString($_GET['verify']);
        ...
        if($verifyCode){
            if(date('Y-m-d',strtotime($verifyCode['createdAt'])) <date('Y-m-d') ){
                ...
                $loadFromUser->update("token",$user_id,array("status"=>1));
                $_SESSION['isVerify']=$user_id;
                redirect_to(url_for("home"));
            }
        } else{
...
// echo Verify::generateLink();

$pageTitle="Verify your accouunt";
?>
<?php require "backend/shared/header.php"; ?>

        <header class="site-header signUpBasicHeader">
            <a href="index.php" class="brand-container" title="Netflix">
            ...
            <a href="signup.php" class="signInLink" >Registration</a>
        </header>
    <section class="simpleContainer" >
        <div class="centerContainer mt">
            
                <div class="regFormContainer no-margin">
                    
                    <div class="stepHeader-body form-container">
                        <?php if(isset($_GET['verify'])){
                            ...
                            <h2>A verification mail has been sent to <?php echo $user['email']; ?>, please check your email to verify your account. </h2>
                        </div>
                        <?php } ?>
