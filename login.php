...
if(is_post_request()){
    if(isset($_POST['submitButton'])){
        ...
        $wasSuccessful=$account->login($email,$pwd);
        if($wasSuccessful){
            session_regenerate_id();
            $_SESSION['isVerify']=$wasSuccessful;
            $userData= $loadFromUser->userData($wasSuccessful);
...
<?php require "backend/shared/header.php"; ?>
        <header class="site-header signUpBasicHeader">
            ...
            <a href="signup" class="signInLink" >Registration</a>
        </header>
            ...
            <form action="<?php echo h($_SERVER['PHP_SELF']); ?>" method="POST" >
                <div class="regFormContainer">
                    <div class="stepHeader-container">
                        <h1 class="stepTitle">Login to Netflix</h1>
                    </div>
                    <div class="stepHeader-body">
                        <div class="group">
                            <?php echo $account->getErrorMessage(Constant::$loginFailed); ?>
